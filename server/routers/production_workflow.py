...
async def get_current_continuity(canvas_id: str):
    item = await get_current_continuity_asset(canvas_id)
    return {"item": item}


@router.get("/main_image/{canvas_id}/current")
async def get_current_main_image(canvas_id: str):
    file_id = await get_current_main_image_file_id(canvas_id)
...
async def set_current_main_image(canvas_id: str, payload: dict):
    ...
    await set_current_main_image_file_id(canvas_id, file_id)
    return {"status": "done", "file_id": file_id}


@router.get("/storyboard/{canvas_id}/current")
async def get_current_storyboard(canvas_id: str):
    item = await get_current_storyboard_plan(canvas_id)
    return {"item": item}


@router.get("/storyboard/{canvas_id}/{storyboard_id}")
async def get_storyboard(canvas_id: str, storyboard_id: str):
    item = await get_storyboard_plan(canvas_id, storyboard_id)
    return {"item": item}


@router.get("/video/brief/{canvas_id}/current")
async def get_current_video_brief_endpoint(canvas_id: str):
    item = await get_current_video_brief(canvas_id)
    return {"item": item}


@router.get("/workflow/{session_id}/pending")
async def get_pending_workflow(session_id: str):
